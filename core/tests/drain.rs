...
use h2proxy_core::conn::{Connection, DrainPolicy, ErrorCode, PREFACE, Settings};
...
impl RawPeer {
    ...
    async fn next(&mut self) -> Option<Frame> {
        loop {
            match self.codec.decode_any(&mut self.buf) {
                Ok(Some(h2proxy_core::frame::Decoded::Frame(frame))) => return Some(frame),
                Ok(Some(h2proxy_core::frame::Decoded::Ignored { .. })) => continue,
                Ok(None) => {}
    ...
    async fn next_matching(&mut self, want: fn(&Frame) -> bool) -> Option<Frame> {
    ...
    async fn send_headers(&mut self, stream_id: u32, fields: &[Header], end_stream: bool) {
    ...
    fn decode(&mut self, block: &Bytes) -> Vec<Header> {
